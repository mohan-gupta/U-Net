...
def center_crop(inp_image, target_image):
    inp_image_size = inp_image.shape[2]
    target_image_size = target_image.shape[2]

    diff = inp_image_size - target_image_size
    diff = diff//2

    cropped = inp_image[:,:, diff:inp_image_size-diff, diff:inp_image_size-diff]
    return cropped

def expansion_block(upsample_layer, conv_layer, inp, concat_inp):
    mask = upsample_layer(inp)
    cropped_inp = center_crop(concat_inp, mask)
    mask = torch.concat([cropped_inp, mask], dim=1)
